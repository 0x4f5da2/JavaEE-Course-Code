...
public class Database {
    ...
    public boolean addComment(String uid, String title, String content) {
        ...
        String sql = "insert into lyTable (user_id, date, title, content) values(?,now(),?,?)";
        try {
            PreparedStatement stmt = connection.prepareStatement(sql);
    ...
    public ArrayList findLyInfo(UsertableEntity entity) {
        String sql = "SELECT * FROM javaee_exp5_1.lytable, javaee_exp5_1.usertable where lytable.user_id = usertable.id";
        if (entity.getPermission() == 0) {
            sql += " and usertable.id = " + entity.getId() + ";";
        }
        try {
            PreparedStatement stmt = connection.prepareStatement(sql);
